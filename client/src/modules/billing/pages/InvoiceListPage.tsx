...
/* ─────────────────────────────────────────────────────────────────
   DEV-ONLY MOCK DATA
   Used only as a fallback while the API isn't wired up yet, or while
   working offline. Remove this block once invoiceService.list() is
   reliably backed by a real endpoint — shipping mock data to a real
   signup is the exact "fake numbers" problem we've been avoiding
...
export default function InvoiceListPage() {
  ...
  /* ── save — same invoiceService used by AiInvoicePage ─────────
     A manually created invoice and an AI-drafted invoice both end up
     here: same endpoint, same shape, no distinction once persisted. */
  const handleSave = useCallback(async () => {
    if (!activeInvoice) return;
    setIsSaving(true);
    try {
      let result: Invoice;
      if (view.name === "create") {
        result = await invoiceService.create(activeInvoice);
        setInvoices((prev) => [result, ...prev]);
      } else {
        result = await invoiceService.update(activeInvoice.id, activeInvoice);
        setInvoices((prev) => prev.map((i) => (i.id === result.id ? result : i)));
      }
      setView({ name: "preview", invoice: result });
    } catch {
      // Keep the user on the current view; InvoiceEditor/Preview's
      // Save button returning to non-saving state is enough signal
      // for now — surfacing a toast here is a good follow-up.
    } finally {
      setIsSaving(false);
      setSaved(true);
      setTimeout(() => setSaved(false), 2500);
          ...
          {view.name === "list" && (
            ...
            >
              ...
              <div className="mb-8 text-center">
                ...
                <p className={`text-sm italic ${muted}`}>
                  {isLoading ? "Loading…" : `${invoices.length} total · click any invoice to view or edit`}
                </p>
              </div>
                ...
                <AnimatePresence initial={false}>
                  {isLoading ? (
                    ...
                    >
                      ...
                      <p className={`text-sm ${muted}`}>Loading your invoices…</p>
                    </motion.div>
                  ) : filtered.length === 0 ? (
                    <motion.div
                      key="empty"
