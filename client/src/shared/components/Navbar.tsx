...
const primaryNav: NavItem[] = [
  ...
  { label: "AI Billing", icon: Zap, href: "/billing/ai", badge: "New" },
];
...
/* ══════════════════════════════════════════
   TOP BAR
   — Mobile/small screens: shows hamburger + TraqBill logo on the left
     (previously empty — desktop's logo lives in Sidebar, which is
     hidden below md, so mobile had no logo anywhere in the fixed header).
   — Desktop (md and up): unchanged, no logo here, Sidebar owns it.
══════════════════════════════════════════ */
export function Topbar({
  ...
  return (
    ...
    >
      {/* ================= MOBILE VIEW ================= */}
      <div className="flex justify-between items-center gap-3 md:hidden w-full relative">
        <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`rounded-lg p-2 ${t.notifBtn(dark)}`}
          >
          ...
          <button
            ...
            className={`h-8 w-8 rounded-full bg-gradient-to-br text-center items-center flex justify-center from-emerald-500 to-teal-400 ring-2 shadow ${t.avatarRing(dark)}`}
        ...
        {isProfileOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 z-70 h-screen bg-black/50 backdrop-blur-sm md:hidden"
            />

            {/* Mobile Dropdown Menu Panel */}
            <div className={`absolute top-14 right-0 w-full rounded-2xl border p-4 shadow-xl flex flex-col gap-4 z-40 animate-in  slide-in-from-top-2 duration-200 ${dark ? "border-gray-800 bg-[#0F1117]" : "border-gray-200 bg-white"}`} 
            >
              {/* User Metadata */}
              <div className="flex items-center gap-2.5 pb-2 border-b border-gray-100 dark:border-gray-800">
                <div className={`h-8 w-8 rounded-full bg-gradient-to-br from-emerald-500 to-teal-400 ring-2 ${t.avatarRing(dark)}`} />
                <div>
              ...
              <div className="relative">
                <Search className={`absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 ${t.textMuted(dark)}`} />
                <input
                  type="text"
                  placeholder="Search invoices, clients…"
                  className={`w-full rounded-xl border pl-9 pr-4 py-2 text-sm outline-none ${t.input(dark)}`}
                />
              </div>

              {/* Mobile Notifications Info */}
              <button className={`flex items-center gap-3 w-full rounded-xl p-2 text-xs font-medium transition-colors ${t.notifBtn(dark)}`}>
                <div className="relative">
                  <Bell className="h-4 w-4" />
                  {notifications > 0 && (
                    <span className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-emerald-500 text-[8px] font-bold text-white grid place-items-center">
                      {notifications}
                    </span>
                  )}
                </div>
              ...
              <Link to="/" className="w-full">
                <button
                  className="flex items-center gap-3 w-full rounded-xl p-2 text-xs font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
                  onClick={() => {
      ...
      {/* ================= DESKTOP VIEW ================= */}
      <div className="ml-auto items-center gap-12 w-auto hidden md:flex">
        {/* Search */}
        ...
        {/* Notifications */}
        <button className={`relative rounded-xl p-2 transition-colors ${t.notifBtn(dark)}`}>
          <Bell className="h-4 w-4" />
          {notifications > 0 && (
            <motion.span
              initial={{ scale: 0 }}
        ...
        <div className="flex items-center gap-2.5 ml-1">
          <div className={`h-8 w-8 rounded-full bg-gradient-to-br from-emerald-500 to-teal-400 ring-2 shadow ${t.avatarRing(dark)}`} />
